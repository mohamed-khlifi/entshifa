  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-md space-y-4 text-primary-foreground">
        <p className="text-3xl font-semibold leading-tight tracking-tight">{t('app.tagline')}</p>
        <p className="text-sm leading-relaxed text-white/85">{t('app.clinicalNote')}</p>
      </div>
      <p className="relative text-xs text-white/70">{t('app.complianceHint')}</p>
    </aside>
  );
}
